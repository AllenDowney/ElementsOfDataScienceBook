...
here to run this notebook on Colab}.from os.path import basename, exists

def download(url): filename = basename(url) if not exists(filename):
...
import utils This chapter introduces Pandas, a Python library that
provides functions for reading and writing data files, exploring and
analyzing data, and generating visualizations. And it provides two new
types for working with data, \passthrough{\lstinline!DataFrame!} and
...
the notebook for this chapter. The NSFG data is available from
...
the terms carefully, but let me draw your attention to what I think is
...
download the
data.download(`https://ftp.cdc.gov/pub/health\_statistics/nchs/datasets/NSFG/stata/'
+
dict\_file)download(`https://ftp.cdc.gov/pub/health\_statistics/nchs/datasets/NSFG/'
+ data\_file) Pandas can read data in most common formats, including
CSV, Excel, and fixed-width format, but it cannot read the data
dictionary, which is in Stata format. For that, we'll use a Python
library called \passthrough{\lstinline!statadict!}. The following cell
installs \passthrough{\lstinline!statadict!} if necessary.try: from
statadict import parse\_stata\_dict except ImportError: !pip install
statadict From \passthrough{\lstinline!statadict!}, we'll import
...
\begin{lstlisting}[style=output]
<statadict.base.StataDict at 0x7f2cf9eb0e90>
\end{lstlisting}
...
\hypertarget{note-for-the-printed-version-of-the-book}{%
\chapter{NOTE: For the printed version of the
book,}\label{note-for-the-printed-version-of-the-book}}

\hypertarget{im-using-iloc-to-show}{%
\chapter{I'm using iloc to show}\label{im-using-iloc-to-show}}

\hypertarget{the-first-5-rows-and-first-9-columns}{%
\chapter{the first 5 rows and first 9
columns}\label{the-first-5-rows-and-first-9-columns}}
...
is another Pandas type, similar to a list. type(nsfg.columns) Based on
the names, you might be able to guess what some of the columns are, but
in general you have to read the documentation. When you work with
datasets like the NSFG, it is important to read the documentation
...
\passthrough{\lstinline!ounces!}. \textbf{Exercise:} The Pandas types we
have seen so far are \passthrough{\lstinline!DataFrame!},
\passthrough{\lstinline!Index!}, and \passthrough{\lstinline!Series!}.
You can find the documentation of these types at:
...
I thought I would mention it. \emph{Elements of Data Science}
